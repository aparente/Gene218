...
## need to open a tk window here to get proper text string sizes
	...
	be passed to this is "scale", which is the scale of the eventual drawing (recommended as ~3.0 for line drawings and 1.0 for vector)'''
        ...
        for s in self:
            ...
            s.cvgcx=s.cvxc-self.cvwidth/2
            if s.hjust=='start': s.cvgcx=s.cvx1
            if s.hjust=='end': s.cvgcx=self.cvwidth-s.cvx2
            s.cvgcy=s.cvyc-self.cvheight/2
            if s.vjust=='top': s.cvgcy-=s.cvyr/2
            if s.vjust=='bottom': s.cvgcy+=s.cvyr/2
...
class VSGitem:
    def __init__(s,parentcanvas,*pr1,**d1):
        ...
        if s.obj=='text':
            ...
            if s.metric.lower()=='tk' and tkFontimported1:
                try:
                    ...
                    s.metric='pil'
                    s.canvas.metric='pil'
            elif s.metric.lower()=='pil' and Imageimported1:
                try:
        ...
        if s.rotate!=0:
            ...
            s.xmin=min(s.x1,s.x2)-s.strokewidth/2
            s.xmax=max(s.x1,s.x2)+s.strokewidth/2
            s.ymin=min(s.y1,s.y2)-s.strokewidth/2
            s.ymax=max(s.y1,s.y2)+s.strokewidth/2
...
def VSGmontage(vl1,columns=3,PanelBuffer=0,align='RU'):
    ...
    if not('P' in align):
        ...
        for i11,vs1 in enumerate(vl1):
            ...
            if 'C' in align: xd1=vs1.xmin-(columnMaxWidth[col11]-(vs1.xmax-vs1.xmin))/2
            if 'U' in align: yd1=vs1.ymax+0
            if 'B' in align: yd1=vs1.ymax-(rowMaxHeight[row11]-(vs1.ymax-vs1.ymin))
            if 'C' in align: yd1=vs1.ymax-(rowMaxHeight[row11]-(vs1.ymax-vs1.ymin))/2
